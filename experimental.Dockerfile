...
RUN --mount=type=cache,target=/root/.cache/go-build CGO_ENABLED=1 GOARCH=amd64 GOOS=linux go build -ldflags='-linkmode external -extldflags "-static"' -a -installsuffix cgo -o /go/bin/go-simple-startpage .
